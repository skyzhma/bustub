...
auto InsertExecutor::Next([[maybe_unused]] Tuple *tuple, RID *rid) -> bool { 
    ...
    while (child_executor_->Next(tuple, rid)) {
        ...
        auto rd = table_info->table_->InsertTuple(meta, *tuple, exec_ctx_->GetLockManager(), exec_ctx_->GetTransaction(), plan_->TableOid());

        // Update index
        for (const auto &index : catalog->GetTableIndexes(table_info->name_)) {
        index->index_->InsertEntry(
            tuple->KeyFromTuple(table_info->schema_, *index->index_->GetKeySchema(), index->index_->GetKeyAttrs()), rd.value(),
